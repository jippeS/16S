...
size_subsample = config['qiime']['size_subsample']
...
os.system("ls [A-Z]*.fastq.gz > list_AZ_fastq_gz_files.txt;ls *.fastq.gz > list_fastq_files.txt")

inputfiles = open("list_AZ_fastq_gz_files.txt" , "r") 

classifier_0 = classifier [:-4]

def step_00_intro (path, nodes, ntasks_per_node, cpus_per_task, identifier, s, qiime_version, path_qiime, path_python, forward_barcodes_file):
    bash_00_intro = ("#!/bin/bash" + "\n"
                       ...
                       + "cd" + s + path[:-1] + "\n\n"
                       + "ls *.fastq.gz > list_fastq_files.txt" + "\n\n"
                       + "mkdir -p" + s + path + "temp" + "\n"   # "mkdir -p temp" + "\n"
                       + "export TMPDIR=" + path + "temp" + "\n\n"
                       + "mkdir -p" + s + identifier  + "\n\n" #"mkdir -p" + s + path + identifier + "\n\n"
                       + "mkdir -p" + s + identifier  + "/raw_data" + "\n\n"
                       + "qiime" + "\n\n"
                       #+ "cp" + s + identifier + "@*_R1_*.fastq.gz" + s + path + identifier + "/raw_data/forward.fastq.gz" + "\n"
                       #+ "cp" + s + identifier + "@*_R2_*.fastq.gz" + s + path + identifier + "/raw_data/reverse.fastq.gz" + "\n"
                       + "cp" + s + "-u" + s + identifier + "@*_R1_*.fastq.gz" + s + path + identifier + "/" + "\n"
                       + "cp" + s + "-u" + s + identifier + "@*_R2_*.fastq.gz" + s + path + identifier + "/" + "\n"
                       + "gunzip" + s + "-k" + s + path + identifier + "/"  + s + identifier + "@*_R1_*.fastq.gz" + "\n"
                       + "gunzip" + s + "-k" + s + path + identifier + "/"  + s + identifier + "@*_R2_*.fastq.gz" + "\n"
                       + "cp" + s + path_python + "qiime_pre_demux.py" + s + path + identifier + "/" + "\n"
                       + "cp" + s + "quality_scores_datasets.txt" + s + path + identifier + "/" + "\n"                       
                       #+ "python3.7" + s + path_python + "qiime2_subsampling_10kseq.py" + "\n\n"                    
                       + "cp" + s + identifier + "@" + forward_barcodes_file + s + path + identifier + "/" + identifier + "@" + forward_barcodes_file + "\n"
                       ...
                       #+ "cp" + s + identifier + "_q_man_01.sh" + s + path + identifier + "/" + "\n" 
                       #+ "cp" + s + identifier + "_q_man_02.sh" + s + path + identifier + "/" + "\n"                    

                       #+ "python3.7" + s + path_python + "qiime2_subsampling_10kseq.py" + "\n\n")
                       #+ "python3.7" + s + path_python + "qiime2_nested_list_transpose.py" + "\n\n") 

    return bash_00_intro

def step_00b_sub_quality (path, nodes, ntasks_per_node, cpus_per_task, identifier, s, qiime_version, path_qiime, path_python, forward_barcodes_file, fastq_sub, name_sub):
    bash_00b_sub_quality = ("#!/bin/bash" + "\n"
                       + "#SBATCH --job-name=" + identifier + "\n"
                       + "#SBATCH --nodes=" + nodes + "\n"
                       + "#SBATCH --ntasks-per-node=" + ntasks_per_node  + "\n"
                       + "#SBATCH --cpus-per-task=" + cpus_per_task + "\n\n"                       
                       + "source activate" + s + path_qiime + "\n\n" #"source activate" + s + path_qiime + "\n\n"
                       + "cd" + s + path[:-1] + "\n\n"
                       + "ls [A-Z]*.fastq.gz > list_AZ_fastq_gz_files.txt" + "\n\n"
                       + "mkdir -p" + s + path + "temp" + "\n"   # "mkdir -p temp" + "\n"
                       + "export TMPDIR=" + path + "temp" + "\n\n"
                       + "mkdir -p" + s + identifier  + "\n\n" #"mkdir -p" + s + path + identifier + "\n\n"
                       + "mkdir -p" + s + identifier  + "/raw_data" + "\n\n"
                       ...
                       + "gunzip" + s + "-k" + s + path + identifier + "/"  + s + identifier + "@*_R2_*.fastq.gz" + "\n"
                       + "cp" + s + path_python + "qiime_pre_demux.py" + s + path + identifier + "/" + "\n"
                       + "cp" + s + path_python + "qiime2_subsampling_quality_check_20200715_1.py" + s + path + identifier + "/" + "\n"     
                       #+ "cp" + s + "quality_scores_datasets.txt" + s + path + identifier + "/" + "\n"                       
                       #+ "python3.7" + s + path_python + "qiime2_subsampling_10kseq.py" + "\n\n"                     
                       + "cp" + s + identifier + "@" + forward_barcodes_file + s + path + identifier + "/" + identifier + "@" + forward_barcodes_file + "\n"
                       + "cd" + s + path + identifier + "\n"
                       ...
                       + "cp" + s + path + identifier + "/" + identifier + "@metadata.txt " + path + str(name_sub) + "kseq-" + identifier + "@metadata.txt" + "\n"
                       + "cp" + s + path + identifier + "/" + identifier + "@metadata.txt " + path + str(name_sub) + "kseq-" + identifier + "@metadata.txt" + "\n"
                       + "cp" + s + path + identifier + "/" + "*kseq-" + identifier + "@*" + s + path + "\n" # + ";iconv -f us-ascii -t utf-8 "  + path + outputfile_10kseq + " > "  + outputfile_10kseq[:-6] + "_utf8.fastq" )
                       + "gzip *.fastq" + "\n"
                       + "cd" + s + path + identifier + "\n"
                       + "chmod -R 777 *.fastq.gz" + "\n"
                       + "python3.6 qiime2_subsampling_quality_check_20200715_1.py"
                       + "cd" + s + path[:-1] + "\n\n")
    ...
    return bash_00b_sub_quality





def step_00a_intro_graphs (path, nodes, ntasks_per_node, cpus_per_task, identifier, s, path_qiime):
    bash_00a_intro_graphs = ("#!/bin/bash" + "\n"
                           ...
                           + "qiime" + "\n\n"
                           + "cd" + s + path[:-1] + "\n\n"
                           + "cp" + s + identifier + "_bash_step_02_qiime_graphs.sh" + s + path + identifier + "/" + "\n")
...
def step_05b_copy_quality_sums (s, path, identifier, path_python):
    bash_05b_copy_quality_sums = ("unzip" + s + path + identifier + "/" + identifier + "_demux.qzv" + s + "-d" + s + path + identifier + "\n"
                                  ...
                                  + '"' + path + identifier + "/" + identifier + "_stats.tsv" + '"' + "; done' _ {} +" + "\n")
...
for identifier in file_list:
    ...
    sbatchfile = (identifier + "_bash_step_01_qiime_scripts.sh")
    sbatch = open(sbatchfile , mode='wb+', newline = None)

    sbatchfile_01 = (identifier + "_bash_step_02_qiime_graphs.sh")
    sbatch_01 = open(sbatchfile_01 , mode='wb+', newline = None)

    sbatchfile_02 = (identifier + "_bash_step_00_qiime_quality.sh")
    ...
    bash_00a_intro_graphs = step_00a_intro_graphs (path, nodes, ntasks_per_node, cpus_per_task, identifier, s, path_qiime)
    bash_00b_sub_quality = step_00b_sub_quality (path, nodes, ntasks_per_node, cpus_per_task, identifier, s, qiime_version, path_qiime, path_python, forward_barcodes_file, fastq_sub, name_sub)


    text_to_write =  ( bash_00_intro + "\n" + bash_01_import + "\n" +  bash_02_demux + "\n"
                       + bash_03_demuxsum + "\n" + bash_04_cutadapt + "\n" + bash_05_trimsum + "\n"
    ...
    text_to_write_1 =  ( bash_00a_intro_graphs + "\n" + bash_21_alpha_rarefaction + "\n" +  bash_22_microbial_diversity + "\n" + bash_23_taxa_barplot + "\n" + "source deactivate" )

    text_to_write_2 =  ( bash_00b_sub_quality + "\n" + "source deactivate" )
    ...
    sbatch.write(bytes(text_to_write, "UTF-8"))

    sbatch_01.write(bytes(text_to_write_1, "UTF-8"))

    sbatch_02.write(bytes(text_to_write_2, "UTF-8"))
