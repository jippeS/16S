...
os.system("ls *.fastq.gz > list_fastq_files.txt")

inputfiles = open("list_fastq_files.txt" , "r") 
...
def step_00_intro (path, nodes, ntasks_per_node, cpus_per_task, identifier, s, qiime_version, path_qiime, path_python, forward_barcodes_file):
    bash_00_intro = ("#!/bin/bash" + "\n"
                       ...
                       + "cp" + s + "-u" + s + identifier + "@*_R1_*.fastq" + s + path + identifier + "/" + "\n"
                       + "cp" + s + "-u" + s + identifier + "@*_R2_*.fastq" + s + path + identifier + "/" + "\n"                       
