...
config.read('/export/jippe/jsil/projects/16S/python_scripts/qiime_settings.ini')
...
os.system("cd" + s + path[:-1] + ";gzip -k *.fastq")  #"gzip fastq files")

os.system("cd" + s + path[:-1] + ";ls *.fastq.gz > " + path_python + "list_fastq_gz_files.txt")  ##"ls [A-Z]*.fastq.gz > list_fastq_gz_files.txt;ls [0-9]*.fastq.gz > subset_fastq_gz_files.txt")

inputfiles = open(path_python + "list_fastq_gz_files.txt" , "r") 
...
def step_00_intro_sub_quality (path, nodes, ntasks_per_node, cpus_per_task, identifier, s, qiime_version, path_qiime, path_python, forward_barcodes_file, fastq_sub, name_sub):
    bash_00_intro_sub_quality = ("#!/bin/bash" + "\n"
                       ...
                       + "cp" + s + "-u" + s + identifier + "*_bash_step_00_*.sh" + s + path + identifier + "/" + "\n" 
...
def step_00_intro_qiime_scripts (path, nodes, ntasks_per_node, cpus_per_task, identifier, s, qiime_version, path_qiime): #old
    bash_00_intro_qiime_scripts = ("#!/bin/bash" + "\n"
                       ...
                       + "mkdir -p" + s + identifier  + "/raw_data" + "\n\n"
                       + "qiime" + "\n\n"
                       + "cp" + s + "-u" + s + identifier + "*_bash_step_01*.sh" + s + path + identifier + "/" + "\n" 
                       + "cd" + s + path + identifier + "\n")
...
def step_00a_intro_graphs (path, nodes, ntasks_per_node, cpus_per_task, identifier, s, path_qiime, path_python):
    bash_00a_intro_graphs = ("#!/bin/bash" + "\n"
                           ...
                           + "cp" + s + "-u" + s + path + identifier + "*_bash_step_02*.sh" + s + path + identifier + "/" + "\n")
...
def step_24_taxa_relative_amounts (s, path, identifier, path_python): 

    bash_24_taxa_relative_amounts = ("mkdir" + s + path + identifier + "/" + identifier + "_taxa_absolute_relative" + "\n" 
                                    + "unzip" + s + path + identifier + "/" + identifier + "_taxa_barplot.qzv" + s + "-d" + s + path + identifier + "/" + identifier + "_taxa_absolute_relative" + "\n"
                                    #+ "cd" + + s + path + identifier + "/" + "\n"  
                                      #echo MAC_CNR1M12_R26RNA > /export3/home/bgeu/transcriptomics/fastq_demux/MAC_CNR1M12_R26RNA/MAC_CNR1M12_R26RNA.id


                                     + "echo" + s + identifier + s + ">" + s + path + identifier + "/" + identifier + "_taxa_absolute_relative" + "/" + identifier + ".id" + "\n"
                                    + "find" + s + path + identifier + s + "-type f -name 'level-*.csv' -exec sh -c" + s + "'for arg do cp --" + s + '"$arg"' + s
                                    + '"' + path + identifier + "/" + identifier + "_taxa_absolute_relative/" + '"' + "; done' _ {} +" + "\n"
                                     + "cd" + s + path + identifier + "/" + identifier + "_taxa_absolute_relative/" + "\n"
                                     + "ls" + s + "level-*.csv > list_csv_files.txt" + "\n" 
                                    + "cp" + s + path_python + "transpose_csv_files.py" + s + path + identifier + "/" + identifier + "_taxa_absolute_relative/" + "\n"
                                    
                                      + "python3.6" + s + path + identifier + "/" + identifier + "_taxa_absolute_relative/transpose_csv_files.py" + "\n" )                                   
                                      #+ "find" + s + path + identifier + s + "-type f -name 'reverse-seven-number-summaries.csv' -exec sh -c" + s + "'for arg do cp --" + s + '"$arg"' + s
                                      #+ '"' + path + identifier + "/" + identifier + "_reverse-seven-number-summaries.csv" + '"' + "; done' _ {} +" + "\n"
                                      # + "cp" + s + path + "quality_scores_datasets.txt" + s + path + identifier + "/" + identifier + "_quality_scores_datasets.txt" + "\n"
                                      #+ "unzip" + s + path + identifier + "/" + identifier + "_denoising_stats.qza" + s + "-d" + s + path + identifier + "\n"
    ...
    return bash_24_taxa_relative_amounts
...
for identifier in file_list:
    ...
    sbatchfile_00 = (path + identifier + "_bash_step_00_qiime_quality.sh") #(identifier + "_bash_step_01_qiime_scripts.sh")
    ...
    bash_24_taxa_relative_amounts = step_24_taxa_relative_amounts (s, path, identifier, path_python) 
...
print("qiime2_step00_bash_quality script created")
