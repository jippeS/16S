...
def qiime2_start_form_1_files():
    ...
    layout = [  [sg.Frame('Step 1 Qiime2 --> Upload and rename files',[
                ...
                [sg.Text('Step 1.2: Upload the fastq or fastq.gz-files and the barcode-file to "/export2/home/microlab/qiime2/pipeline_qiime2-2019-10/raw_illumina_data_gz/"', font=("Helvetica", 11))],            
...
def main():
    ...
    while True:
        ...
        python_script = "/export/jippe/jsil/projects/16S/python_scripts/"

        if window == window_2 and event == 'qiime settings':
            os.system("gedit " + "{}qiime_settings.ini".format(python_script))
        ...
        if window == window_2 and event == 'Create bash-script 00':
            os.system("python3.7 " + "{}qiime2_step00_bash_quality_20211028.py".format(python_script))

        if window == window_3 and event == 'Create bash-script 01':
            os.system("python3.7 " + "{}qiime2_step01_bash_qiime2_20211028.py".format(python_script))

        if window == window_4a and event == 'Create bash-script 02':
            os.system("python3.7 " + "{}qiime2_step02_bash_graphs_20211028.py".format(python_script))

        if window == window_4b and event == 'Create bash-script 03':
            os.system("python3.7 " + "{}qiime2_step03_bash_merge_20211117.py".format(python_script))
