...
def change_names(folder_path):
    mapping_file = folder_path + "sampleName_clientId.txt"
    fastq_dir = Path(folder_path)
    ...
    with open(mapping_file) as f:
        for line in f:
            ...
            for i in [0, 1]:
                old_file = fastq_dir / f"{old_name}{suffixes_old[i]}"
                new_file = fastq_dir / f"{new_name}{suffixes_new[i]}"
                ...
                if old_file.exists():
                    ...
                    if dry_run:
                        print(f"DRY-RUN: {old_file.name} → {new_file.name}")
                    else:
...
def list_files_in_folder(folder_path):
    ...
    return_list = set()
    for i in list_1:
        if i != "folder.sh" and i.endswith(".gz"):
...
config["forw"] = str(config["forw"])
config["reve"] = str(config["reve"])

folder_path = config["inputdir"]
# config["inputdir"] = config["inputdir"] + config["forw"] + "_" + config["reve"] + "/"
# config["naming_convention"] = config["naming_convention"] + "_" + config["forw"] + "_" + config["reve"]

change_names(folder_path)
...
rule unpack_and_get_manifest:
    ...
    message:
        ...
        "@#"
    shell:
        # "cp {config[inputdir]}*sampleName* {config[inputdir]}input/{config[naming_convention]}@metadata.txt;"
        "python python_scripts/metadata.py --forward_primer={config[forward]} --reverse_primer={config[reverse]} --output={output.metadata} --input='{files_list}';"
...
rule Demultiplex:
    ...
    message:
        ...
        "@#"
    shell:
        # "cp {config[inputdir]}input/*metadata* {config[inputdir]}input/{config[naming_convention]}@metadata.txt;"
