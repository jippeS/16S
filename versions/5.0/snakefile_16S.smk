...
classifier_name132 = config["classifier132"].split("/")[-1][:-4]
...
rule Import_data:
    output:
        config["inputdir"] + "1_import_data_done.txt"
    conda:
        config["condaenvs"] + config["qiime_v2"]
    params:
        inputpath = config["inputdir"] + folder_name,
        output_pair_end = outputdir + "Artifacts_qza/" + config["naming_convention"] + "_PairEndSequences.qza"
    benchmark:
    ...
    message:
        ...
        "   --output-path {params.output_pair_end} "
    ...
    shell:
        ...
        "python3 python_scripts/pre_demux.py --inputdir={config[inputdir]} --forward={config[forward]}  --reverse={config[reverse]};"
        ...
        "sbatch bash_scripts/calc/import_data.sh {params.inputpath}input/raw_data/ {params.output_pair_end};"
        "python3 {config[tooldir]}wetsus_packages/wait_file.py {params.output_pair_end};"
        "touch 1_import_data_done.txt;"
        "mv 1_import_data_done.txt {config[inputdir]};"
...
rule Demultiplex:
    ...
    params:
        ...
        cores = 8,
        input_import = outputdir + "Artifacts_qza/" + config["naming_convention"] + "_PairEndSequences.qza"
    benchmark:
    ...
    message:
        ...
        "@#"
    shell:
        "sbatch bash_scripts/calc/demultiplex.sh {params.metadata} {params.input_import} {params.p_error_rate} {output.demux} {output.untrimmed} {params.cores};"
...
rule classifying_reads_132:
    input:
        denois = rules.denoising_paired.output.representative,
        classify = rules.export_classified.output
    output:
        outputdir + "Artifacts_qza/" + config["naming_convention"] + "_" + classifier_name132 + "_132.qza"
    conda:
        config["condaenvs"] + config["qiime_v2"]
    params:
        threads = 8,
        batch_size = 200
    benchmark:
        outputdir + "benchmarks/Classification_132.txt"
    message:
        "@#"
        "Classify reads:   "
        "qiime feature-classifier classify-sklearn "
        "   --i-classifier {config[classifier132]} "
        "   --i-reads {input.denois} "
        "   --o-classification {output} "
        "   --p-n-jobs {params.threads} "
        "   --p-reads-per-batch {params.batch_size}"
        "@#"
    shell:
        "sbatch bash_scripts/calc/classify_sklearn.sh {config[classifier132]} {input.denois} {output} {params.threads} {params.batch_size};"
        "python3 {config[tooldir]}wetsus_packages/wait_file.py {output};"

rule visualize_classification_132:
    input:
        rules.classifying_reads_132.output
    output:
        outputdir + "Visualization_qzv/" + config["naming_convention"] + "_" + classifier_name132 + "_132.qzv"
    conda:
        config["condaenvs"] + config["qiime_v2"]
    benchmark:
        outputdir + "benchmarks/visualize_classification_132.txt"
    message:
        "@#"
        "Visualize classification:   "
        "qiime metadata tabulate "
...
rule export_classified_132:
    input:
        rules.classifying_reads_132.output
    output:
        outputdir + "export/" + config["naming_convention"] + "_taxonomy_132.tsv"
    params:
        first_output = outputdir + "export/taxonomy.tsv"
    conda:
        config["condaenvs"] + config["qiime_v2"]
    benchmark:
        outputdir + "benchmarks/export_classified_132.txt"
    message:
        "@#"
        "Exporting classifications: "
        "qiime tools export "
...
rule Taxonomy_analysis_132:
    input:
        taxa = rules.classifying_reads_132.output,
        table = rules.denoising_paired.output.table
    output:
        outputdir + "Visualization_qzv/" + config["naming_convention"] + "_taxonomy_barplot_132.qzv"
    conda:
        config["condaenvs"] + config["qiime_v2"]
    message:
        "@#"
...
rule return_zipped_results:
    input:
        ...
        vis_classify132= rules.visualize_classification_132.output,
        export_classify132= rules.export_classified_132.output,
        taxonomy132 = rules.classifying_reads_132.output,
        taxonomy_analysis_132= rules.Taxonomy_analysis_132.output
    ...
    shell:
        ...
        "cp {input.taxonomy123} {params.zip_folder};"
