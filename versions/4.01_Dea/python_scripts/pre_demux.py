...
class PreDemux:
    def __init__(self):
        ...
        self.forward_out = args.forward_out
        self.reverse_out = args.reverse_out
    ...
    def opening_files(self):
        ...
        fastq_R1_file = self.forward
        fastq_R2_file = self.reverse

        # Retrieve locations for the future forward and reverse files.
        output_forward_file = self.forward_out
        output_reverse_file = self.reverse_out

        # Opens the files containing the sequences.
        with gzip.open(fastq_R1_file, "rt") as handle_fwd:
            with gzip.open(fastq_R2_file, "rt") as handle_rev:
                # Opens the files where the data is getting written to.
                with gzip.open(output_forward_file, "wt") as output_forward:
                    with gzip.open(output_reverse_file, "wt") as output_reverse:
...
def argparser():
    ...
    parser.add_argument("--forward", help="input file must be a fasta file.")
    parser.add_argument("--forward_out", help="output file must be a fasta file.")
    parser.add_argument("--reverse", help="input file must be a fasta file.")
    parser.add_argument("--reverse_out", help="output file must be a fasta file.")
    parser.add_argument("--config", help="is the config file")
